...
void get_input(_q *data)
{
    uint8_t send[4] = { 0x03, 0, 0, INTERIM_LEN_ADDR };
    uint16_t input_len = 0;

    spi_transfer(send, (uint8_t *) &input_len, 4, 2, 0, 0);
    send[0] = 0x03;
    send[1] = 0x00;
    send[2] = 0x00;
    send[3] = INTERIM_ADDR;
    spi_transfer(send, (uint8_t *) input, 4, input_len, 1, 1);
}
...
int main(void)
{
    ...
    GPIO_PIN6);
    GPIO_selectInterruptEdge(GPIO_PORT_P5,
    GPIO_PIN6,
    ...
    GPIO_setAsInputPinWithPullDownResistor(GPIO_PORT_P5,
    GPIO_PIN7);
    GPIO_selectInterruptEdge(GPIO_PORT_P5,
    GPIO_PIN7,
    ...
    int num_of_layer = sizeof(extended_layer) / sizeof(extended_layer[0]);
    int num_of_valid_layer = 0;
    int i, l;
    ...
    for (l = 1; l < num_of_layer; l++)
    {
        ...
        for (i = 0; i < MAX_NUM_OF_LAYER_DIM; i++)
        {
            if (extended_layer[l][i] != 0)
            {
                num_of_layer_item += 1;
            }
        }

        if (num_of_layer_item >= 4)
        {
            num_of_valid_layer += 1;
        }
    }
    ...
    while (1)
    {
        ...
        for (i = 0; i < MAX_INPUT_DIM; i++)
        {
            input_dim[i] = extended_layer[0][i];
        }

        feed_forward(input, input_dim, extended_weights, extended_biases,
                     extended_layer, num_of_valid_layer + 1);

        uint16_t interim_len = output_dim[0] * output_dim[1] * output_dim[2];
        ...
        spi_transfer((uint8_t *) interim_len, &recv, 2, 0, 0, 0);
        send[0] = 0x02;
        send[1] = 0x00;
        send[2] = 0x00;
        send[3] = INTERIM_ADDR;
        spi_transfer(send, &recv, 4, 0, 0, 0);
        spi_transfer((uint8_t *) input_addr, &recv, interim_len * 2, 0, 1, 0);
